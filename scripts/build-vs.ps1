...
if (!((Test-Path -Path $DLL_PATH) -and (Test-Path -Path $DLL_PATH2))) {
  ...
  py -3.13 -m pip install Cython setuptools

  $env:VSPYTHON_PATH = (py -3.13 -c "import sys; import os; print(os.path.dirname(sys.executable))")
  echo "VSPYTHON_PATH=$env:VSPYTHON_PATH"
